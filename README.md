...
- **Tools / Skills**(可折叠):每项一个复选框**热插拔**开关 —— 关掉的工具对模型隐藏、关掉的技能移出系统提示词索引;插件工具(如 rag 的 `search_docs`、`sandbox`)也在此列出;开启 **evolve** 后,模型自建的工具(`create_tool` 产物)在造出的**当轮即时刷新**到这里,无需重连
